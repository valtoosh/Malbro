// app/apply/confirmation/[uid]/page.tsx
...
import { getSubmissionByPublicUid } from '@/db/queries/submissions';
...
export default async function ConfirmationPage({ params }: { params: Promise<{ uid: string }> }) {
  ...
  const sub = await getSubmissionByPublicUid(db, uid);
  if (!sub) notFound();
    ...
    <PageShell schedule={`Schedule R-7 · Form 042-A · Filing ${uid.slice(0, 8)}`}>
      <div className="max-w-[760px] mx-auto">
        ...
        <Card className="mt-10 bg-paper-2 p-8">
          ...
          <div className="mt-6">
            <Tag variant={sub.status === 'approved' ? 'default' : sub.status === 'rejected' ? 'alert' : 'muted'}>
          ...
          <dl className="grid grid-cols-2 gap-y-3 gap-x-6 mt-8 text-body">
            ...
            <dd><a className="underline break-all" href={sub.tweetUrl} target="_blank" rel="noopener noreferrer">{sub.tweetUrl}</a></dd>
            <dt className="font-mono text-caption uppercase">Filed</dt>
            <dd>{sub.createdAt.toISOString().slice(0, 10)}</dd>
          </dl>

          {sub.rejectionReason && (
            <div className="mt-6 p-4 border-2 border-stamp-red">
              <p className="font-mono text-eyebrow uppercase mb-2 text-stamp-red">REJECTION REASON</p>
        ...
        <div className="mt-10 flex items-center gap-6 flex-wrap">
          <Link href="/wall" className="font-mono text-eyebrow uppercase tracking-[0.12em] underline underline-offset-[3px]">
            WALL OF GRANTS
          </Link>
          <Stamp label={sub.status === 'approved' ? 'APPROVED' : sub.status === 'rejected' ? 'REJECTED' : 'UNDER REVIEW'} rotate={-9} />
        </div>
      </div>
    </PageShell>
  );
