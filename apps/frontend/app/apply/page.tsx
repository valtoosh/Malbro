// apps/frontend/app/apply/page.tsx
import Link from 'next/link';
import { cookies } from 'next/headers';
import { PageShell, Button, Input, Card, Tag } from '@marlbro/ui';
import { submitApplication } from './actions';
import { APPLICANT_COOKIE_NAME } from '@/lib/api';
...
export default async function ApplyPage({
  ...
  const applicantRaw = jar.get(APPLICANT_COOKIE_NAME)?.value;
  let applicant: { handle: string; twitterId: string } | null = null;
  if (applicantRaw) {
    try {
      applicant = JSON.parse(applicantRaw) as { handle: string; twitterId: string };
    } catch {
      applicant = null;
    }
  }

  // Stub mode is determined by the backend; we can show the dev label if env var not set on frontend
  const isStubMode = !process.env.TWITTER_CLIENT_ID;
      ...
      <div className="max-w-[760px] mx-auto">
        ...
        {!applicant && (
          <Card className="mt-10 p-8 bg-paper-2">
            ...
            <Button asChild>
              <Link
                href={`/api/twitter/login?next=${encodeURIComponent(`/apply${sp.bounty ? `?bounty=${sp.bounty}` : ''}`)}`}
              >
                SIGN IN WITH X
              </Link>
            </Button>
            ...
            <form action={submitApplication} className="space-y-6" encType="multipart/form-data">
              ...
              <label className="block">
                ...
                <Input
                  name="walletAddress"
                  required
                  placeholder="5n3X9pK4rT2QmL8vWYxZdNbF7HjAuC1k7p4q"
                />
              ...
              <label className="flex items-start gap-3 cursor-pointer">
                <input
                  ...
                  required
                  className="mt-1.5 w-4 h-4 accent-marlbro"
                />
